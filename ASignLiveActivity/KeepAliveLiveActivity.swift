	...
	var body: some WidgetConfiguration {
		ActivityConfiguration(for: KeepAliveAttributes.self) { context in
			VStack(spacing: 8) {
				...
				_ControlsRow(isPaused: context.state.isPaused)
			}
			.padding()
...
/// Pause/resume for downloads. Only rendered on iOS 17+ where interactive
/// Live Activity buttons exist; hidden for owners that have no pause state.
private struct _ControlsRow: View {
	let isPaused: Bool?

	var body: some View {
		if #available(iOS 17.0, *), let isPaused {
			HStack(spacing: 10) {
				if isPaused {
