...
struct Options: Codable, Equatable {
        ...
        /// If ASign should remove the app after signed it in the Downloaded Apps options
    var removeApp: Bool
    /// If ASign should only modify and no signing
    var onlyModify: Bool
        /// If ASign copy things should start in the last used location instead of Documents dir
        var useLastExportLocation: Bool?
        /// If ASign should use Zip or ZIPFoundation
    ...
    /// If ASign should use background audio
    var backgroundAudio: Bool
    /// If ASign should show logs when signing
    var signingLogs: Bool
    /// If ASign should notify when download is completed
    var notifications: Bool
        ...
        /// Inject a bundled shim that forces document pickers into copy mode
        /// so sandboxed apps can read picked files. Optional for decode
        /// compatibility with previously saved options.
        var fixFilePicker: Bool?
        /// Derive fresh keychain-access-groups for the signed app from its
        /// provisioning profile instead of keeping the original ones.
        var keychainIsolation: Bool?
        /// Raw XML plist merged over the app's Info.plist (Info.plist editor).
        var infoPlistOverridesXML: String?
        /// PPQ protection style: "plain" appends the random string, "prefixed"
        /// re-roots the identifier under asign. first.
        var ppqStyle: String?
        // default
