...
//                    .disabled(!options.ppqProtection)
        ...
        NBSection(.localized("Advanced")) {
            ...
            _toggle(.localized("Fix File Picker"),
                    systemImage: "folder.badge.gear",
                    isOn: Binding(
                        get: { options.fixFilePicker == true },
                        set: { options.fixFilePicker = $0 }
                    ),
                    temporaryValue: temporaryOptions?.fixFilePicker
            )
            _toggle(.localized("Keychain Isolation"),
                    systemImage: "key.horizontal",
                    isOn: Binding(
                        get: { options.keychainIsolation == true },
                        set: { options.keychainIsolation = $0 }
                    ),
                    temporaryValue: temporaryOptions?.keychainIsolation
            )
        } footer: {
            Text(.localized("Keychain isolation derives fresh keychain groups from the provisioning profile, so re-signed apps no longer share secrets with their original bundle."))
        }
