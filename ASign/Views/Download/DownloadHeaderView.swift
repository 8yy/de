        ...
        var body: some View {
                VStack(alignment: .leading, spacing: 4) {
                        HStack {
                                ...
                                Button {
                                    if isPaused {
                                        DownloadManager.shared.resumePausedDownload(download)
                                    } else {
                                        DownloadManager.shared.pauseDownload(download)
                                    }
                                    ASHaptic.tap()
                                } label: {
                                    Image(systemName: isPaused ? "arrow.clockwise.circle.fill" : "pause.circle.fill")
                                        .font(.title3)
                        ...
                        HStack {
                                Text(verbatim: "\(Int(overallProgress * 100))%")
                                        .contentTransition(.numericText())
                                if speed > 0, !isPaused {
                                    Text(verbatim: speed.formattedSpeed)
                                        .contentTransition(.numericText())
                                }
                                if isPaused {
                ...
                .onReceive(download.$speed) { self.speed = $0 }
                .onReceive(download.$isPaused) { self.isPaused = $0 }
        }
