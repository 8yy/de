...
final class SigningHandler: NSObject {
        ...
        func modify() async throws {
                ...
                // ASign: vault auto-inject rules and the optional FilePickerFix
                // shim are resolved here, against the app's *original* bundle id,
                // before anything mutates the session options further.
                let originalBundleID = infoDictionary["CFBundleIdentifier"] as? String
                _options = await TweakLibrary.resolvedSessionOptions(_options, bundleIdentifier: originalBundleID)
        ...
        try await _removeCodeSignature(for: movedAppPath)
                ...
                // ASign: keychain isolation derives fresh keychain-access-groups
                // from the new provisioning profile and merges them into the
                // entitlements handed to zsign.
                _applyKeychainIsolationIfNeeded()
...
extension SigningHandler {
        private func _modifyDict(using infoDictionary: NSMutableDictionary, with options: Options, to app: URL) async throws {
                ...
                // ASign: raw Info.plist overrides from the editor. Applied last so
                // they always win over the toggles above.
                if let xml = options.infoPlistOverridesXML, !xml.isEmpty,
                   let data = xml.data(using: .utf8),
                   let overrides = (try? PropertyListSerialization.propertyList(from: data, format: nil)) as? [String: Any] {
    ...
    // MARK: - ASign pipeline hooks
    
    /// Merges keychain-access-groups derived from the provisioning profile into
    /// the entitlements passed to zsign. Runs on the session options copy.
    private func _applyKeychainIsolationIfNeeded() {
        guard _options.keychainIsolation == true else { return }
        ...
        if let entitlementsURL = MachOEntitlements.entitlementsFile(
            customEntitlements: _options.appEntitlementsFile,
            provisionURL: provision,
            newBundleID: bundleID,
            keychainIsolation: true
        ) {
