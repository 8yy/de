...
final class KeepAliveActivityController {
	...
	private let _queue = DispatchQueue(
		label: "nya.asami.asign.live-activity",
		qos: .userInitiated
	)
	...
	func report(_ owner: BackgroundAudioManager.Owner, speedText: String?) {
		_merge(owner) {
			let value = (speedText?.isEmpty == false) ? speedText : nil
			guard $0.speedText != value else { return }
			$0.speedText = value
		}
	}

	// Pause state for the download controls in the Dynamic Island.
	func report(_ owner: BackgroundAudioManager.Owner, isPaused: Bool?) {
		_merge(owner) {
